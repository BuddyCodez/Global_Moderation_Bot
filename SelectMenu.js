                ...
                const modal = new Modal() // We create a Modal
                  ...
                  .addComponents(
                    ...
                    new TextInputComponent() // We create a Text Input Component
                      ...
                      .setStyle("SHORT") //IMPORTANT: Text Input Component Style can be 'SHORT' or 'LONG'
                      .setPlaceholder("Enter a Url of icon either type my to use profile icon.")
                      .setRequired(false)
                  );
                showModal(modal, {
       ...
       if (interaction.customId == `Player_${user.id}`) {
        ...
        if (data[0] == `add`) {
          const embed = new EmbedBuilder()
            .setDescription(`<a:tick:1012933003120476181> Added to Favourite List!`)
            .setColor(0xff0019)
            .setFooter({
          ...
          const MusicData = await Music.findOne({ User: user.id }) ||
         ...
         if (data[0] == "remove") {
             ...
             const MusicData =
               ...
               new Music({ User: user.id });
             if (!MusicData.FavoriteList.includes(player.currentTrack.info.nametitle))
             return;
