  ...
  async execute(client, interaction) {
    ...
    if (interaction.isButton()) {
      if (interaction.customId.startsWith("_Accept.")) {
        ...
        if (!interaction.member.roles.cache.has(AdminRole)) return interaction.editReply({ content: "You are not an admin, you are not allowed to perform this!", ephemeral: true });
        ...
        client.users.cache.get(target).send({ embeds: [UserBanEmbed] }).then(() => console.log("sent message to user")).catch(e => console.error(e));
        ...
        const LogEmbed = {
          ...
          color: 0x00ff00,
          footer: { text: `${client.users.cache.get(target).tag} is banned from ${BanSize} guilds` },
        };
        if (NotBannable.length > 0) {
          LogEmbed.description = `Total Guilds: ${Guilds.size}\nBanned from Total Guilds: ${BanSize}\nNot Bannable from Total Guilds: ${NotBannable.join(", ")}`;
         }
        ...
        await client.db.get("global_LogChannel").then(async (ch) => {
          ...
          await msg.edit({ components: [AdminBtnRow(true)] }).then(async () => {
            ...
            client.users.cache.get(ReporterUser).send({ embeds: [ReporterEmbed] }).then(() => console.log("<<<<< sent message to Reporter >>>>")).catch(e => console.error(e));
        ...
        await msg.edit({ components: [AdminBtnRow(true)] }).then(() => {
            showModal(modal, {
